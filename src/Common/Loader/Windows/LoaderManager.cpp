...
Loader::LoaderManager::~LoaderManager()
{
  for (std::map<std::string, DynamicLibraryLoader *>::iterator it = this->dlloader_.begin(); it !=  this->dlloader_.end(); ++it)
    delete (*it).second;
  ...
  if (table && folder)
    ...
    for(it = folderRow.begin(); it != folderRow.end(); it++)
		if (!it->second.at(1).compare("Libraries"))
			path = it->second[0];
    if (path == "")
      std::cerr << "LoadManager : No path specified in database. Path is current directory." << std::endl;
...
bool			Loader::LoaderManager::loadLibrary(std::string const &path, std::string const &name)
{
  ...
  try
    {
      tmp = new DynamicLibraryLoader(path + name + ".dll");
